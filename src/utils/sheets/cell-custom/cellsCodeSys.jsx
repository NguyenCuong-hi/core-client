import { getMiddleCenterBias, GridCellKind, TextCellEntry, useTheme, DataEditor } from '@glideapps/glide-data-grid';
...
export const Editor = (p) => {
  ...
  const debounceCallGetCodeHelp = useMemo(
    () =>
      debounce(async (key) => {
        ...
        try {
          const result = await SearchCategory(1, 'fmOperationRegister', '', '', 1, 1000, key);
  ...
  const filteredData = useMemo(() => {
    ...
    const normalizeText = (text) => (typeof text === 'string' || typeof text === 'number' ? text.toString().toLowerCase().trim() : '');

    const search = normalizeText(searchText);
    const propertiesToSearch = ['code', 'value', 'type'];
  ...
  useEffect(() => {
    setCacheData((prev) => {
      const safePrev = prev ?? [];
      const existingItemSeqs = new Set(safePrev.map((item) => item.id));
      const newData = extraData.filter((item) => !existingItemSeqs.has(item.id));
      return [...safePrev, ...newData];
    });
  }, [extraData, setCacheData]);
...
export const CellsCodeSys = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'cells-code-sys',
  draw: (args, cell) => {
  ...
  provideEditor: () => ({
    ...
    styleOverride: {
      ...
      maxHeight: 'unset',
      overflow: 'auto',
      boxShadow: '0 0 0 1px #d1d9e0b3,  rgba(50, 50, 93, 0.25) 0px 50px 100px -20px, rgba(0, 0, 0, 0.3) 0px 30px 60px -30px'
    },
    disablePadding: true
  }),
  onPaste: async (v, d) => {
    const normalizeText = (text) =>
      typeof text === 'string' || typeof text === 'number' ? text.toString().trim().toLowerCase().normalize('NFC') : '';
