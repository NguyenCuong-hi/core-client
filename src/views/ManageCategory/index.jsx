...
const ManageCategory = ({ canCreate }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await SearchCategoryBy(data);
  ...
  const onClickSave = useCallback(async () => {
    showLoader();
    const requiredColumns = ['form', 'type', 'value', 'code'];

    const commonColumns = ['id', 'description', 'form', 'type', 'value', 'code'];
    ...
    const categoryNew = resulA.map((item) => {
      return {
        ...item,
    ...
    const categoryEdit = resulU.map((item) => {
      return {
        ...item,
      };
    });
    ...
    const dataCategory = [...categoryNew, ...categoryEdit];

    try {
      const promises = [];

      if (dataCategory.length > 0) promises.push(CreateCategoryService(dataCategory));
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      if (selected.length > 0) promises.push(DeleteCategoryBy(ids));
  ...
  return (
    <>
      <div className="h-full pt-4">
        <CategoryAction titlePage={t('Danh mục hệ thống')} keyword={keyword} setKeyword={setKeyword} onClickSearch={onSearch} onClickSave={onClickSave} onClickDelete={onClickDelete} onClickImport={onClickImport} />
