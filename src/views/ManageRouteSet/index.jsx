...
import RouteInfomationQuery from './query/RouteInfomationQuery';
...
const ManageRouteSetPage = ({ canCreate, canEdit, canDelete, canView }) => {
  ...
  const defaultColsOperation = useMemo(() => [
    {
      title: '',
      id: 'Status',
      kind: 'Text',
  ...
  }, [formBasic, gridDataCategory, gridDataOpRoute, isAPISuccess, routeId, gridDataOperationRework, gridDataOpIndicate]);
...
useEffect(() => {
    ...
    try {
      ...
      if (result.success) {
        ...
        notify({
          type: 'error',
          message: 'Loi',
          description: 'Có lỗi xảy ra khi thêm dòng mới!'
        });
      }
    } catch (error) {
  ...
  const onClickDelete = useCallback(async () => {
    ...
    try {
      ...
      const response = await DeleteRouteById(routeId);
      ...
      <div className="h-full mt-4">
        ...
        <Splitter className="w-full h-full border border-gray-300 ">
          ...
          <SplitterPanel size={75} minSize={10}>
            <RouteInfomationQuery formBasic={formBasic} />
